...
fs.writeFileSync('skill.csv', util.format("%s\t%s\t%s\n", 'skillId', 'name', 'gameDesc'));

jsonSkill.forEach(objSkill => {
  fs.appendFileSync('skill.csv', util.format("%s\t%s\t%s\n", objSkill.skillId, objSkill.name, objSkill.gameDesc.replace(/[\n\[\]]/g, '')));
});
